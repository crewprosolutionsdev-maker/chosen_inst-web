...
export async function createCheckout(req, res, next) {
  try {
    ...
    const settings = await StoreSettings.findOne({ key: 'checkout' }).lean() || { shipping: { flatRate: 0, freeThreshold: 0, pickupEnabled: true, pickupLabel: 'Retiro a coordinar' }, transfer: { enabled: true }, mercadoPago: { enabled: true } };
    ...
    const shippingCost = pickup || (settings.shipping.freeThreshold > 0 && subtotal >= settings.shipping.freeThreshold) ? 0 : settings.shipping.flatRate;
    ...
    const order = await Order.create({ customer, items: orderItems, subtotal, shipping: { method: pickup ? 'pickup' : 'delivery', label: pickup ? settings.shipping.pickupLabel : 'Envío a domicilio', cost: shippingCost }, total: subtotal + shippingCost, paymentMethod });

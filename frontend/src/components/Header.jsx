  ...
  return <>
    ...
    <header>
      <div className="header-main container">
        ...
        <div className="utilities"><Link to="/admin" className="account" aria-label="Ir al panel de administración"><UserRound/><small>ADMINISTRACIÓN</small></Link><button className="cart-trigger" onClick={onCartOpen}><ShoppingBag/><span><small>CARRITO ({cartSummary.count})</small><b>{money(cartSummary.total)}</b></span></button></div>
      </div>
      <nav className={menuOpen ? 'open' : ''}><div className="container nav-inner">{[['/','INICIO'],['/productos','PRODUCTOS'],['/contacto','CONTACTO'],['/admin','ADMIN']].map(([to,label]) => <NavLink key={to} to={to} onClick={() => setMenuOpen(false)}>{label}</NavLink>)}</div></nav>
    </header>
  </>;
}
